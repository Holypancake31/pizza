...
// проверка пароля
    ...
    string[] pizza_names = pizzas.Select(x => x["Название"].ToString()).ToArray();
...
// -1 чтобы все красиво записалось. Ячейки начинаются с 0. Чтоб в конечном итоге не было нуля пишется - 1.
        ...
        var need_napitki = EsenUtils.sprositDaNet("Вам нужны напитки? ");
        int pizzas_napitki = 0;
        if (need_napitki == "да")
        {
            pizzas_napitki =
                EsenUtils.PrintSprositSpisok("У нас есть", ["Соки", "Газировки", "Коктели"], null);
        }
        if (pizzas_napitki == 1)
        {
...
// Здесь вычисляется цена с учетом скидки.
        ...
        if (koktel_number > 0) ;
        {
            ...
            Console.WriteLine("Вы выбрали этот коктейль: " + koktel_name + " Его цена: " + koktel_cost);
        }
        ...
        if (zazirovka_number > 0) ;
        {
            ...
            Console.WriteLine("Вы выбрали эту газировку: " + zazirovka_name + " Еe цена: " + zazirovka_cost);
        }
        if (sok_number > 0) ;
        {
            ...
            Console.WriteLine("Вы выбрали этот сок: " + sok_name + " Его цена: " + sok_cost);
        }
        ...
        if (pizza_soys == "да")
        {
            Console.WriteLine("Вы выбрали этот соус: " + sauce_name + " Его цена: " + sauce_cost);
            Console.WriteLine("Вы выбрали эту пиццу: " + pizza_name + " Ее количество:" + pizzas_count +
                              " Цена каждой пиццы:" + pizza_price);
            ...
            var new_pizza_nasklade = nasklade - pizzas_count;
